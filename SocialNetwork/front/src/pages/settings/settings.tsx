import React from "react";
import { NavLink, Outlet } from "react-router-dom";

const settingsNavItems = [
    {
        to: "privacy",
        label: "Privacy",
        description: "Who can see your profile and posts",
        icon: (
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z"
                />
            </svg>
        ),
    },
    {
        to: "accountinfo",
        label: "Account Info",
        description: "Email, password, and profile details",
        icon: (
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"
                />
            </svg>
        ),
    },
];

export const Settings: React.FC = () => {

    return (
        <div className="space-y-6">
            {/* page header */}
            <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 shadow-lg shadow-violet-500/30">
                    <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a7.722 7.722 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z"
                        />
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                    </svg>
                </div>
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-white">Settings</h1>
                    <p className="mt-1 text-sm text-slate-400">
                        Manage your account preferences, privacy, and security.
                    </p>
                </div>
            </div>

            <div className="flex flex-col gap-6 lg:flex-row lg:gap-8">
                {/* settings nav */}
                <nav className="shrink-0 lg:w-64">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-2 shadow-lg shadow-violet-950/20 backdrop-blur-xl">
                        <p className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            Preferences
                        </p>
                        <ul className="flex flex-col gap-1 sm:flex-row sm:flex-wrap lg:flex-col">
                            {settingsNavItems.map((item) => (
                                <li key={item.to} className="flex-1 lg:flex-none">
                                    <NavLink
                                        to={item.to}
                                        className={({ isActive }) =>
                                            [
                                                "group flex flex-col gap-0.5 rounded-xl px-3 py-3 transition-all duration-200 sm:flex-1 lg:flex-none",
                                                isActive
                                                    ? "bg-gradient-to-r from-violet-600/80 to-fuchsia-600/60 text-white shadow-lg shadow-violet-900/40"
                                                    : "text-slate-400 hover:bg-white/5 hover:text-white",
                                            ].join(" ")
                                        }
                                    >
                                        <span className="flex items-center gap-3">
                                            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/5 transition group-hover:bg-white/10">
                                                {item.icon}
                                            </span>
                                            <span className="text-sm font-semibold">{item.label}</span>
                                        </span>
                                        <span className="hidden pl-12 text-xs opacity-80 lg:block">{item.description}</span>
                                    </NavLink>
                                </li>
                            ))}
                            <li className="flex-1 lg:flex-none">
                                <NavLink
                                    to="another"
                                    className={({ isActive }) =>
                                        [
                                            "group flex flex-col gap-0.5 rounded-xl px-3 py-3 transition-all duration-200 sm:flex-1 lg:flex-none",
                                            isActive
                                                ? "bg-gradient-to-r from-violet-600/80 to-fuchsia-600/60 text-white shadow-lg shadow-violet-900/40"
                                                : "text-slate-400 hover:bg-white/5 hover:text-white",
                                        ].join(" ")
                                    }
                                >
                                    <span className="flex items-center gap-3">
                                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/5 transition group-hover:bg-white/10">
                                            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                                            </svg>
                                        </span>
                                        <span className="text-sm font-semibold">Another</span>
                                    </span>
                                    <span className="hidden pl-12 text-xs opacity-80 lg:block">Additional settings page</span>
                                </NavLink>
                            </li>
                        </ul>
                    </div>
                </nav>

                {/* settings content */}
                <div className="min-w-0 flex-1">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-inner shadow-black/20 backdrop-blur-xl sm:p-6">
                        <Outlet/>
                    </div>
                </div>
            </div>
        </div>
    );
};
